...
class FlightController:
    ...
    def __init__(self, link_uri):
        ...
        self.pidctrlr_alt.setDGain(160)
...
if __name__ == '__main__':
    ...
    if len(available) > 0:
        ...
        le.plotter1.updateY(le.pidctrlr_alt.getErrorAccum(),le.pidctrlr_alt.getIncAccum(),le.baroReadings, le.batteryReadings)
        print "Time under AutoPilot = ", le.pidctrlr_alt.getControlTime(), " seconds"
        le.plotter1.plot()
        errorFile = open("error.txt", "wb")
        for data in le.pidctrlr_alt.getErrorAccum():
            string = str(data) + ","
