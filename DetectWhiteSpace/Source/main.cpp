...
void sendDataToServer(int best_sub_matrix);
void setComp(int x);
...
int run_all(char args[])
{
	...
	while (1)
	{
		...
		{
			cv::threshold(im_gray, img_bw, 128.0, 255.0, THRESH_BINARY); // convert image to binary
			img_bw_comp = complementImage(img_bw, img_bw.rows, img_bw.cols); // complement binary image
		}
		...
		if (comp == 1)
		{
			...
			morphOps(img_bw_comp);
			imshow("Transformed Image", img_bw_comp);
		}
		else if (comp == 0)
		{
			namedWindow("Transformed Image", WINDOW_NORMAL);
			morphOps(img_bw);
			imshow("Transformed Image", img_bw);
		}
		else
		{
			namedWindow("Transformed Image", WINDOW_NORMAL);
			morphOps(threshold);
			imshow("Transformed Image", threshold);
		}
		...
		sendDataToServer(pc.best_sub_matrix);
...
void createTrackbars()
{
	...
	createTrackbar("COMP", trackbarWindowName, &comp, 2, on_trackbar);
